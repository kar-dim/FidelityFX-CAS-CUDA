...
__global__ void cas_rgb(cudaTextureObject_t texObj, const float sharpenStrength, const float contrastAdaption, unsigned char* casOutputR, unsigned char* casOutputG, unsigned char* casOutputB, const unsigned int height, const unsigned int width)
{
...
//main CAS kernel (ReShade based)
__global__ void cas_interleaved(cudaTextureObject_t texObj, const float sharpenStrength, const float contrastAdaption, unsigned char* casOutputRGB, const unsigned int height, const unsigned int width)
{
	const int x = blockIdx.x * blockDim.x + threadIdx.x;
	const int y = blockIdx.y * blockDim.y + threadIdx.y;
	const int outputIndex = (y * width) + x;
	...
	casOutputRGB[outputIndex * 3] = static_cast<unsigned char>(clamp(sharpenedValues.x * 255.0f, 0.0f, 255.0f));
	casOutputRGB[outputIndex * 3 + 1] = static_cast<unsigned char>(clamp(sharpenedValues.y * 255.0f, 0.0f, 255.0f));
	casOutputRGB[outputIndex * 3 + 2] = static_cast<unsigned char>(clamp(sharpenedValues.z * 255.0f, 0.0f, 255.0f));
}